...
void AutoWah::prepareToPlay(double sr, int samplesPerBlock) {
  ...
  setFrequency(2000.f);
  bandpass.setCutoffFrequency(digitalCutoffFrequency / _sampleRate);
}

void AutoWah::setBandpassParameters() {
  const auto lfoValue = _lfo.getNextSample(); 
  lfoValue - (lfoValue + 1.f) / 2.f;
  bandpass.setCutoffFrequency((digitalCutoffFrequency + (2000.f * lfoValue)) /
                              _sampleRate);
}
